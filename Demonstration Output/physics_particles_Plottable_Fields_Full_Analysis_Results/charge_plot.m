% MATLAB script for plotting field: charge
hist_data = load('charge_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f8 = figure('Name', 'charge Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('charge Histogram');
xlabel('charge Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('charge_full_analysis.txt');
