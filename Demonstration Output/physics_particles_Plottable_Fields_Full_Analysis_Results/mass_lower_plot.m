% MATLAB script for plotting field: mass_lower
hist_data = load('mass_lower_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f4 = figure('Name', 'mass_lower Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('mass_lower Histogram');
xlabel('mass_lower Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('mass_lower_full_analysis.txt');
