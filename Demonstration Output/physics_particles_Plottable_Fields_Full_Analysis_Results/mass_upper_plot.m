% MATLAB script for plotting field: mass_upper
hist_data = load('mass_upper_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f0 = figure('Name', 'mass_upper Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('mass_upper Histogram');
xlabel('mass_upper Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('mass_upper_full_analysis.txt');
