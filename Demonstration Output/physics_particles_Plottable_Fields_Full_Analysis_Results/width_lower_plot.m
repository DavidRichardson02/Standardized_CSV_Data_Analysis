% MATLAB script for plotting field: width_lower
hist_data = load('width_lower_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f1 = figure('Name', 'width_lower Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('width_lower Histogram');
xlabel('width_lower Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('width_lower_full_analysis.txt');
