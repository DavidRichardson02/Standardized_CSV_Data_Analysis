% MATLAB script for plotting field: width_upper
hist_data = load('width_upper_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f3 = figure('Name', 'width_upper Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('width_upper Histogram');
xlabel('width_upper Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('width_upper_full_analysis.txt');
