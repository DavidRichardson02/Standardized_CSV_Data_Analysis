% MATLAB script for plotting field: airTemperature
hist_data = load('airTemperature_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f1 = figure('Name', 'airTemperature Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('airTemperature Histogram');
xlabel('airTemperature Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('airTemperature_full_analysis.txt');
