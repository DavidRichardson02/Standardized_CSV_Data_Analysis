% MATLAB script for plotting field: barometricPressure
hist_data = load('barometricPressure_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f14 = figure('Name', 'barometricPressure Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('barometricPressure Histogram');
xlabel('barometricPressure Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('barometricPressure_full_analysis.txt');
