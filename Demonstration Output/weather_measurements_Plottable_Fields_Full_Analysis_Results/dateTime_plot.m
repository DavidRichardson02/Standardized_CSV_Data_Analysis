% MATLAB script for plotting field: dateTime
hist_data = load('dateTime_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f16 = figure('Name', 'dateTime Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('dateTime Histogram');
xlabel('dateTime Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('dateTime_full_analysis.txt');
