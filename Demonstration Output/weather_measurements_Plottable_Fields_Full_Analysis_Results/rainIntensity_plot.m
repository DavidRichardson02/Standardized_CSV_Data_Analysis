% MATLAB script for plotting field: rainIntensity
hist_data = load('rainIntensity_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f6 = figure('Name', 'rainIntensity Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('rainIntensity Histogram');
xlabel('rainIntensity Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('rainIntensity_full_analysis.txt');
