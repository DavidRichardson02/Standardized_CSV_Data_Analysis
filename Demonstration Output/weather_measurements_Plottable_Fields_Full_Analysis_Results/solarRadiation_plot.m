% MATLAB script for plotting field: solarRadiation
hist_data = load('solarRadiation_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f13 = figure('Name', 'solarRadiation Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('solarRadiation Histogram');
xlabel('solarRadiation Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('solarRadiation_full_analysis.txt');
