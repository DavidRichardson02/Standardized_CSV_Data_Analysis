% MATLAB script for plotting field: wetBulbTemperature
hist_data = load('wetBulbTemperature_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f0 = figure('Name', 'wetBulbTemperature Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('wetBulbTemperature Histogram');
xlabel('wetBulbTemperature Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('wetBulbTemperature_full_analysis.txt');
