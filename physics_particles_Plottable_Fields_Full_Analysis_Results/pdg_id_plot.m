% MATLAB script for plotting field: pdg_id
hist_data = load('pdg_id_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f6 = figure('Name', 'pdg_id Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('pdg_id Histogram');
xlabel('pdg_id Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('pdg_id_full_analysis.txt');
