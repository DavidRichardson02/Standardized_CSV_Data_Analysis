% MATLAB script for plotting field: heading
hist_data = load('heading_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f2 = figure('Name', 'heading Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('heading Histogram');
xlabel('heading Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('heading_full_analysis.txt');
