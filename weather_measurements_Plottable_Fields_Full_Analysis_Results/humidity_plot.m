% MATLAB script for plotting field: humidity
hist_data = load('humidity_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f15 = figure('Name', 'humidity Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('humidity Histogram');
xlabel('humidity Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('humidity_full_analysis.txt');
