% MATLAB script for plotting field: intervalRain
hist_data = load('intervalRain_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f3 = figure('Name', 'intervalRain Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('intervalRain Histogram');
xlabel('intervalRain Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('intervalRain_full_analysis.txt');
