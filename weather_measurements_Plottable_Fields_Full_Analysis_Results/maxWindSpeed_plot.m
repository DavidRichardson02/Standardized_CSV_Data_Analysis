% MATLAB script for plotting field: maxWindSpeed
hist_data = load('maxWindSpeed_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f8 = figure('Name', 'maxWindSpeed Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('maxWindSpeed Histogram');
xlabel('maxWindSpeed Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('maxWindSpeed_full_analysis.txt');
