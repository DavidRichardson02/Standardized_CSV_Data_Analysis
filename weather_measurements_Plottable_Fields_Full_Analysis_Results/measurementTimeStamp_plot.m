% MATLAB script for plotting field: measurementTimeStamp
hist_data = load('measurementTimeStamp_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f9 = figure('Name', 'measurementTimeStamp Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('measurementTimeStamp Histogram');
xlabel('measurementTimeStamp Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('measurementTimeStamp_full_analysis.txt');
