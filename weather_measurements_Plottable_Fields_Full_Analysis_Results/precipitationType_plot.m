% MATLAB script for plotting field: precipitationType
hist_data = load('precipitationType_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f10 = figure('Name', 'precipitationType Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('precipitationType Histogram');
xlabel('precipitationType Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('precipitationType_full_analysis.txt');
