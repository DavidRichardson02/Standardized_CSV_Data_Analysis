% MATLAB script for plotting field: stationBatteryLife
hist_data = load('stationBatteryLife_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f12 = figure('Name', 'stationBatteryLife Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('stationBatteryLife Histogram');
xlabel('stationBatteryLife Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('stationBatteryLife_full_analysis.txt');
