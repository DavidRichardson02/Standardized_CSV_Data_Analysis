% MATLAB script for plotting field: windDirection
hist_data = load('windDirection_histogram.txt');
bin_centers = (hist_data(:,1) + hist_data(:,2))/2;
counts = hist_data(:,3);
f4 = figure('Name', 'windDirection Histogram');
bar(bin_centers, counts, 1.0, 'FaceColor',[0.7 0.7 0.7]);
title('windDirection Histogram');
xlabel('windDirection Values');
ylabel('Frequency');
grid on;
hold on;
% Load analysis to overlay normal distribution
A = fileread('windDirection_full_analysis.txt');
